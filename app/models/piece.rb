...
class Piece < ApplicationRecord
  ...
  def opposition_piece?(x_end, y_end, id = nil, color = nil)
    ...
    if id.nil?
      ...
      true
    elsif self.id != id && white != color # ex: King moving to square above pawn, and when performing king.check?, pawn will recognize there is an opposition piece, making the vertical move false
      true
    else
      false
    end
    ...
    until back_to_start
      ...
      if current_position == [new_x, new_y]
        ...
        places_between << if x_distance == y_distance
                            ...
                            [current_piece.x_position, new_y]
                          else
  ...
  def name
    "#{piece_type}_#{white ? 'white' : 'black'}"
  end
end
